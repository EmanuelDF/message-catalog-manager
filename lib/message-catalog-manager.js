...
var buildData = function (data, dataNamedInsertsPattern, namedInserts, positionalInserts) {
    var outputData = {};
    ...
    if (dataNamedInsertsPattern) {
        const regex = new RegExp(dataNamedInsertsPattern);
        const filteredNamedInserts = Object.keys(namedInserts)
            .filter(key => key.match(regex))
            .reduce((newObject, key) => {
                newObject[key] = namedInserts[key];
                return newObject;
            }, {});
        if(Object.keys(filteredNamedInserts).length !== 0) {
            outputData = filteredNamedInserts;
        }
    }

    //Build data object based on data descriptor
    for (var key in data) {
        outputData[key] = applyInserts(data[key], namedInserts, positionalInserts);
    }
...
MessageCatalogManager.prototype.getMessage = function (catalogName, code, namedInserts, positionalInserts, language, verbose) {
    ...
    var dataNamedInsertsPattern;
    if (message.dataNamedInsertsPattern) {
        dataNamedInsertsPattern = message.dataNamedInsertsPattern;
        delete message.dataNamedInsertsPattern;
    }

    //apply Inserts to detail
    message.data = buildData(message.data, dataNamedInsertsPattern, namedInserts, positionalInserts);
